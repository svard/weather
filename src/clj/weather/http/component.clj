...
(defrecord HttpServer [conf db websocket]
  ...
  (start [component]
    ...
    (defroutes app-routes
      ...
      (GET "/temperature/line/:type" [] (make-resource rest/line-data (:db db)))
      (GET "/seasons" [] (make-resource rest/seasons (:db db)))
      (GET "/precipitation" [] (make-resource rest/precipitation (:db db)))
      (GET "/precipitation/humidity" [] (make-resource rest/humidity (:db db)))
      (GET "/precipitation/bar" [] (make-resource rest/bar-data (:db db)))
;      (GET "/chart/:type" [] (make-resource rest/chart-data (:db db)))
